...
function filterRun(){
  ...
  var itemCheck = [];
  $("input[name='itemSelector']:checked").each(function (i, n) {
      itemCheck.push(n.value);
  });
  ...
  for (var i = 0; i < servantData.length ; i++){
    ...
    /*if ( !(itemCheck == servantData[i].item || itemCheck == 'all') ){
      continue;
    }*/
    if (itemCheck.indexOf(servantData[i].item) < 0) {
        continue;
    }
...
function putResult(servant){
  ...
  $("." + servant.rarity).children("." + servant.class).append("<span>" + servant.name + "(" + itemName[servant.item] + ")</span><br/>")
}
